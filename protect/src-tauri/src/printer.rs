...
pub fn create_printer_dc(printer_name: &str, settings: &PrintSettings) -> std::result::Result<HDC, String> {
...
pub fn start_doc(hdc: HDC) -> std::result::Result<(), String> {
...
pub fn end_doc(hdc: HDC) -> std::result::Result<(), String> {
    unsafe {
        if EndDoc(hdc) <= 0 {
...
pub fn print_page(
    ...
    let grayscale = color_mode == DMCOLOR_MONOCHROME;
    for y in 0..ih {
        for x in 0..iw {
...
pub fn print_image(
    ...
    if let Err(e) = start_doc(hdc) {
        delete_dc(hdc);
        return Err(e);
    }
    if let Err(e) = print_page(hdc, image_path, settings.color_mode) {
        abort_doc(hdc);
        delete_dc(hdc);
        return Err(e);
    }
    if let Err(e) = end_doc(hdc) {
        delete_dc(hdc);
        return Err(e);
    }
    delete_dc(hdc);
